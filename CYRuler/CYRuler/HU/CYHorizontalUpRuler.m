...
#import "CYRulerHUHeadCell.h"
#import "CYRulerHUFootCell.h"

extern BOOL didImplementationEnd;

@interface CYHorizontalUpRuler()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic,assign) NSInteger scale;

@end

@implementation CYHorizontalUpRuler
...
- (void)setPointer:(UIView *)pointer {
    ...
    CGRect frame = [self convertRect:CGRectMake(1, self.rulerModel.pointerOffset-orginframe.size.height/2.0 + self.rulerModel.defaultScale / self.rulerModel.valueOfUnitScale * CYRulerUnitPX, orginframe.size.width, orginframe.size.height) toView:self.superview];
    pointer.frame = frame;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        CYRulerHUHeadCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell0"];
        if (!cell) {
            cell = [[CYRulerHUHeadCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell0"];
        ...
        CYRulerHUFootCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell1"];
        if (!cell) {
            cell = [[CYRulerHUFootCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell1"];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        cell.contentView.transform = CGAffineTransformMakeRotation(M_PI/2);
        cell.paramModel = self.paramModel;
        ...
        CYRulerHUCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
        if (!cell) {
            cell = [[CYRulerHUCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        cell.contentView.transform = CGAffineTransformMakeRotation(M_PI/2);
        cell.dataModel = self.rulerModel.dataSources[indexPath.row];
