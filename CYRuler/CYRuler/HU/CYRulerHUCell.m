...
@interface CYRulerHUCell()

@property (nonatomic,strong) NSMutableArray *dataSources;

@end

@implementation CYRulerHUCell
...
- (void)addView {
    self.valueLable = [[UILabel alloc] initWithFrame:CGRectMake(CYRulerCellWith / 2.0, rulerHeight - 16 -CYRulerLongScaleLength, CYRulerCellWith, 10)];
...
- (void)calculateDataModel {
    ...
    model.addPoint = CGPointMake(CYRulerCellWith, rulerHeight - 1);
    [self.dataSources addObject:model];
    
    for (int i = 0; i < 11; i++) {
        CYRulerDataSourceModel *model = [CYRulerDataSourceModel model];
        model.movePoint = CGPointMake(i * 7, rulerHeight - 1);
        model.addPoint = CGPointMake(i * 7, rulerHeight - 1 - (i % 10 == 0?CYRulerLongScaleLength:CYRulerShortScaleLength));
        [self.dataSources addObject:model];
    }
}
