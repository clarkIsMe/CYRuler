...
@implementation CYRulerHUHeadCell
...
- (void)addView {
    self.valueLable = [[UILabel alloc] initWithFrame:CGRectMake(pointerOffsetLeft - CYRulerCellWith / 2.0, rulerHeight - 16 -CYRulerLongScaleLength, CYRulerCellWith, 10)];
...
- (void)calculateDataModel {
    ...
    model0.movePoint = CGPointMake(0, rulerHeight - 1);
    model0.addPoint = CGPointMake(pointerOffsetLeft, rulerHeight - 1);
    CYRulerDataSourceModel *model1 = [CYRulerDataSourceModel model];
    model1.movePoint = CGPointMake(pointerOffsetLeft, rulerHeight - 1);
    model1.addPoint = CGPointMake(pointerOffsetLeft, rulerHeight - 1 - CYRulerLongScaleLength);
